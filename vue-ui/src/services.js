...
const TRUCK_SERVICE_URL = "http://localhost:3000/locateTrucks";
const NEAREST_VEHICLE_SERVICE_URL = "http://localhost:3001/findNearestVehicle";


export function getAccounts() {
    var url = encodeURI(`${ACCOUNT_URL}?where=SLA__c='Platinum'&fields=Name,BillingAddress`);
    return axios.get(url).then(response => response.data);
}

export function getLocalTrucks(searchCity) {
    var url = encodeURI(`${TRUCK_SERVICE_URL}`);
